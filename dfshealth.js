...
(function () {
  ...
  function render() {
    var helpers = {
      ...
      'helper_dir_status-cli': function (chunk, ctx, bodies, params) {
        var j = ctx.current();
        for (var i in j) {
          chunk.write(i + ':' + j[i] + '(' + params.type + ')');
        }
      }
    };

    var base = dust.makeBase(helpers);
    var tmpl = fs.readFileSync('dfshealth-cli.dust.txt', 'utf-8');
    var code = dust.compile(tmpl, 'dfshealth', false);
    dust.loadSource(code);
  ...
  var BEANS = [
    {"name": "nn",      "url": NNADDR + "/jmx?qry=Hadoop:service=NameNode,name=NameNodeInfo"},
    {"name": "nnstat",  "url": NNADDR + "/jmx?qry=Hadoop:service=NameNode,name=NameNodeStatus"},
    {"name": "fs",      "url": NNADDR + "/jmx?qry=Hadoop:service=NameNode,name=FSNamesystemState"},
    {"name": "mem",     "url": NNADDR + "/jmx?qry=java.lang:type=Memory"},
    {"name": "startup", "url": NNADDR + "/startupProgress"}
  ];
